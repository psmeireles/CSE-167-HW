...
OBJObject::OBJObject(char* filepath)
{
	...
	glBufferData(GL_ARRAY_BUFFER, vertices2.size()*sizeof(glm::vec3), vertices2.data(), GL_STATIC_DRAW);
	...
	while (c1 != EOF) {
		if (c1 == 'v') {
			...
			if (c2 == 'n') {
				...
				this->normals2.push_back(glm::vec3(x, y, z));
				this->colors.push_back(glm::vec3((x + 1.0f) / 2.0f, (y + 1.0f) / 2.0f, (z + 1.0f) / 2.0f));
			}
			else if (c2 == ' ') {
				fscanf(fp, " %f %f %f %f %f %f", &x, &y, &z, &r, &g, &b);
				this->vertices2.push_back(glm::vec3(x, y, z));
...
void OBJObject::shiftAndResizeModel()
{
	...
	for (int i = 0; i < vertices2.size(); i++) {
		vertices2[i].x -= avg_x;
		vertices2[i].x /= max_coord;
		vertices2[i].y -= avg_y;
		vertices2[i].y /= max_coord;
		vertices2[i].z -= avg_z;
		vertices2[i].z /= max_coord;
	}
